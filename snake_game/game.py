...
class SnakeGame:
    ...
    def _spawn_food(self):
        ...
        empty_cells = [cell for cell in all_cells if cell not in self.snake]
    
        if empty_cells:
            return random.choice(empty_cells)
    
        # Game is won (entire grid filled) - return current food position
        return self.food if hasattr(self, 'food') else (0, 0)
...
def get_safe_neighbors(self, pos):
    ...
    neighbors = get_neighbors(pos, self.grid_rows, self.grid_cols)
    return [n for n in neighbors if self.is_position_safe(n)]